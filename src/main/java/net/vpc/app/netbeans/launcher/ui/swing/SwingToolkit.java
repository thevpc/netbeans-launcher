...
import net.vpc.app.netbeans.launcher.ui.swing.utils.SwingUtils2;
...
public class SwingToolkit {
    ...
    public SwingToolkit(Component parent) {
        this.parent = parent;
        lang = ResourceBundle.getBundle("net.vpc.app.netbeans.launcher.ui.Messages");
    }
    ...
    private static class ButtonLabelMouseAdapter extends MouseAdapter {
        ...
        public ButtonLabelMouseAdapter(String icon, ButtonAction action) {
            this.action = action;
            normalIcon = MainWindowSwingHelper.loadIcon(icon + ".png", 24);
...
//            selectedIcon = new ImageIcon(SwingUtils2.grayScaleImage2(normalIcon.getImage()));
    ...
    public class Message {
        ...
        public String getText() {
            ...
            while (m.find()) {
                String v = params.get(m.group());
                if (v == null) {
                    v = "?";
                }
                m.appendReplacement(sb, Pattern.quote(v));
            }
            m.appendTail(sb);
            return sb.toString();
        }
...
//        jLabel.setFont(new Font(Font.MONOSPACED,Font.BOLD,12));
    ...
    public JComponent createIconButton(String icon, String tooltip, ButtonAction action, boolean compact) {
        if(true){
            ...
            compact = true;
            ImageIcon imageIcon = MainWindowSwingHelper.loadIcon(icon + ".png", compact ? 16 : 32);
